...
export const getPosts = (req,res) =>{
    ...
    if(!token) return res.status(401).json("Not logged out");
    ...
    const q = `SELECT p.*, u.id AS userId,name,profilePic FROM post AS p JOIN users AS u ON (u.id = p.userId) LEFT JOIN relationships 
